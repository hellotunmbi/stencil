...
export const parseToModule = (config: d.Config, compilerCtx: d.CompilerCtx, buildCtx: d.BuildCtx, tsSourceFile: ts.SourceFile, typeChecker: ts.TypeChecker, collection: d.CollectionCompilerMeta) => {
  ...
  const visitNode = (node: ts.Node) => {
    if (ts.isClassDeclaration(node)) {
      ...
      parseStaticComponentMeta(config, compilerCtx, typeChecker, classNode, moduleFile, compilerCtx.nodeMap);
