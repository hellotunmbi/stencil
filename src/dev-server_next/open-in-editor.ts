...
export async function serveOpenInEditor(devServerConfig: d.DevServerConfig, req: d.HttpRequest, res: http.ServerResponse) {
  let status = 200;
  ...
  try {
    if (devServerConfig.editors.length > 0) {
      await parseData(devServerConfig, req, data);
...
async function parseData(devServerConfig: d.DevServerConfig, req: d.HttpRequest, data: d.OpenInEditorData) {
  ...
  try {
    const stat = await util.stat(data.file);
