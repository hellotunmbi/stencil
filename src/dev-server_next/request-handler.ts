...
import { isDevClient, sendMsg, stat } from './dev-server-utils';
...
export function createRequestHandler(devServerConfig: d.DevServerConfig) {
  ...
  return async function(incomingReq: http.IncomingMessage, res: http.ServerResponse) {
    try {
      ...
      if (isDevClient(req.pathname) && devServerConfig.websocket) {
        return serveDevClient(devServerConfig, req, res);
      }
      ...
      try {
        req.stats = await stat(req.filePath);
        ...
        if (req.stats.isDirectory()) {
          return serveDirectoryIndex(devServerConfig, req, res);
        }
