...
export async function serveFile(devServerConfig: d.DevServerConfig, req: d.HttpRequest, res: http.ServerResponse) {
  try {
    if (util.isSimpleText(req.filePath)) {
      // easy text file, use the internal cache
      let content = await util.readFile(req.filePath, 'utf8');
